...
class BrainD:
    ...
    def main(self):
        ...
        Fs_train = np.array([i for i in pool.imap(self.process, train_dfs)], dtype=np.float64).reshape(410, 15, 15)
        Fs_test = np.array([i for i in pool.imap(self.process, test_dfs)], dtype=np.float64).reshape(410, 15, 15)
        ...
        Xs_train = np.array([i for i in pool.imap(self.scaling, train_dfs)], dtype=np.float64).reshape(-1, 15)
        Xs_test = np.array([i for i in pool.imap(self.scaling, test_dfs)], dtype=np.float64).reshape(-1, 15)
